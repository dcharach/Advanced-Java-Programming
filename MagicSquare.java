...
public class MagicSquare {
	public static void main(String[] args) {
		...
		// call method to check if rows, columns, and diagonals add up to the same
		// number
	...
	/**
	 * the getNumbers method asks the user to enter the numbers in the order they
	 * want to place them in the magic square and checks that the input is valid (no
	 * number is used twice, and the numbers are between 1 and 9)
	 * 
	...
	public static int[][] getNumbers() {
		...
		try {// create two dimensional array filled with numbers the user entered
			for (int x = 0; x < 3; x++) {
				for (int c = 0; c < 3; c++) {
					System.out.println("please enter the number you want in the magic square " + "for row " + (x + 1)
							+ " column " + (c + 1));
					...
					while (number > 9 | number < 1) {
						System.out
								.println("You must enter a number from 1 to 9." + "\nPlease enter a different number");
						number = keyboard.nextInt();
					}
	...
	/**
	 * The displayArray method displays the array holding the magic square values
	 * 
	 * @param magicSquare
	 *            holds the magicSquare array
	 */
